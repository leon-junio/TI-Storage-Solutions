...
public class ProdutoService {
	...
	public Object getHistorico(Request request, Response response) {
		try {
			...
			return utils.LeonAPI.stringToJson("<tr><td colspan=\"4\">Não há registros no Estoque.</td></tr>");
		}
	}
		...
		try {
			...
			if (produtos.size() > 0) {
				...
				for (model.Produto produto : produtos) {
					...
					resp += "<a href=\"./home-form-produto.html?id=" + produto.getEstoque() + "&&idproduto="
							+ produto.getIdProduto() + "\">";
					resp += "<button type=\"button\" class=\"btn btn-outline-primary\"><i class=\"fas fa-eye\"></i></button>";
					resp += "</a>";
					resp += "<button type=\"button\" class=\"btn btn-outline-danger\" onclick=\"deletarProduto('/produto/deletar/"
							+ produto.getEstoque() + "/" + produto.getIdProduto()
	...
	public Object carregar(Request request, Response response) {
		try {
			...
			resp += "<label for=\"nome\" class=\"form-label\">Nome do Produto</label> <input type=\"text\" name=\"nome\" id=\"nome\" class=\"form-control\" value=\""
					+ obj.getNome() + "\">";
			resp += "</div><div class=\"col-6\">";
			resp += "<label for=\"descricao\" class=\"form-label\">Descrição do Produto</label> <input type=\"text\" name=\"descricao\" id=\"descricao\" class=\"form-control\" value=\""
					+ obj.getDescricao() + "\">";
			resp += "</div></div>";

			resp += "<div class=\"row mb-3\"><div class=\"col-6\">";
			resp += "<label for=\"quantidade\" class=\"form-label\">Quantidade</label> <input type=\"number\" name=\"quantidade\" id=\"quantidade\" class=\"form-control\" value=\""
					+ obj.getQuantidade() + "\">";
			resp += "</div><div class=\"col-6\">";
			resp += "<label for=\"codBarras\" class=\"form-label\">Código de Barras</label> <input type=\"text\" name=\"codBarras\" id=\"codBarras\" class=\"form-control\" value=\""
					+ obj.getCodigoBarras() + "\">";
			resp += "</div></div>";
			...
			resp += "<option value=\"AMPOLA\">AMPOLA</option>" + "<option value=\"BARRA\">BARRA</option>"
					+ "<option value=\"BOMBONA\">BOMBONA</option>" + "<option value=\"BISNAGA\">BISNAGA</option>"
					+ "<option value=\"CAIXA\">CAIXA</option>" + "<option value=\"CARRETEL\">CARRETEL</option>"
					+ "<option value=\"CENTO\">CENTO</option>" + "<option value=\"CENTIMETRO\">CENTIMETRO</option>"
					+ "<option value=\"CONJUNTO\">CONJUNTO</option>" + "<option value=\"CHAPA\">CHAPA</option>"
					+ "<option value=\"DUZIA\">DUZIA</option>" + "<option value=\"DOSE\">DOSE</option>"
					+ "<option value=\"EMBALAGEM\">EMBALAGEM</option>"
					+ "<option value=\"ENGRADADO\">ENGRADADO</option>" + "<option value=\"ESTOJO\">ESTOJO</option>"
					+ "<option value=\"FARDO\">FARDO</option>" + "<option value=\"FRASCO\">FRASCO</option>"
					+ "<option value=\"GALÃO\">GALÃO</option>" + "<option value=\"GRAMA\">GRAMA</option>"
					+ "<option value=\"JOGO\">JOGO</option>" + "<option value=\"LATA\">LATA</option>"
					+ "<option value=\"LITRO\">LITRO</option>" + "<option value=\"METRO\">METRO</option>"
					+ "<option value=\"MILHEIRO\">MILHEIRO</option>" + "<option value=\"M²\">M²</option>"
					+ "<option value=\"M³\">M³</option>" + "<option value=\"PACOTE\">PACOTE</option>"
					+ "<option value=\"PALLET\">PALLET</option>" + "<option value=\"PAR\">PAR</option>"
					+ "<option value=\"PEÇA\">PEÇA</option>" + "<option value=\"QUILO\">QUILO</option>"
					+ "<option value=\"RESMA\">RESMA</option>" + "<option value=\"ROLO\">ROLO</option>"
					+ "<option value=\"SACHE\">SACHE</option>" + "<option value=\"SACO\">SACO</option>"
					+ "<option value=\"SERINGA\">SERINGA</option>" + "<option value=\"TONELADA\">TONELADA</option>"
					+ "<option value=\"TUBO\">TUBO</option>" + "<option value=\"UNIDADE\">UNIDADE</option>"
			...
			resp += "</div><div class=\"col-6\">";
			resp += "<label for=\"fabricacao\" class=\"form-label\">Ano de Fabricação</label> <input type=\"date\" name=\"fabricacao\" id=\"fabricacao\" class=\"form-control\" value=\""
					+ dateFormated.format(obj.getFabricacao()) + "\">";
			resp += "</div></div>";

			resp += "<div class=\"row mb-3\"><div class=\"col-6\">";
			resp += "<label for=\"validade\" class=\"form-label\">Ano de Validade</label> <input type=\"date\" name=\"validade\" id=\"validade\" class=\"form-control\" value=\""
					+ dateFormated.format(obj.getValidade()) + "\">";
			resp += "</div><div class=\"col-6\">";
			resp += "<label for=\"marca\" class=\"form-label\">Marca do Produto</label> <input type=\"text\" name=\"marca\" id=\"marca\" class=\"form-control\" value=\""
					+ obj.getMarca() + "\">";
			resp += "</div></div>";

			resp += "<div class=\"row mb-3\"><div class=\"col-6\">";
			resp += "<label for=\"peso\" class=\"form-label\">Peso</label> <input type=\"number\" min=\"0\" max=\"100\" step=\".01\" name=\"peso\" id=\"peso\" class=\"form-control\" value=\""
					+ obj.getPeso() + "\">";
			resp += "</div></div>";
	...
	public Object deletar(Request request, Response response) {
		...
		try {
			if (daop.delete(idProduto)) {
				response.status(200);
				return "<script>alert('produto foi excluído com sucesso!'); window.location.href = '"
						+ app.Aplicacao.url + "/pages/home-produtos.html?id=" + idEstoque + "'</script>";
			} else {
				response.status(203);
			...
			response.status(203);
			return "<script>alert('Não foi possível excluir o produto pois o mesmo possuí relações internas estabelescidas em entradas e retiradas'); window.location.href = '"
					+ app.Aplicacao.url + "/pages/home-produtos.html?id=" + idEstoque + "'</script>";
		}
	}
	...
	public Object atualizar(Request request, Response response) {
		...
		try {
			...
			return "<script>alert('Não foi possível atualizar o produto!'); window.location.href = '"
					+ app.Aplicacao.url + "/pages/home-form-produto.html?id=" + estoque + "&&idproduto=" + idProduto
					+ "'</script>";
		}
				...
				if (produtos.isEmpty()) {
					...
					for (Produto prod : produtos) {
						data += "<option value=\"" + prod.getIdProduto() + "\"><data value=\"" + prod.getQuantidade()
								+ "\">" + prod.getNome() + " - " + prod.getQuantidade()
								+ " unidade(s) </data></option>\n";
					}
				}
				response.status(200);
	...
	public Object retirada(Request request, Response response) {
		...
		try {
			...
			int qtd = Integer.parseInt(request.queryParams("quantidade"));
			java.util.Date dataRet = dateFormated.parse(request.queryParams("dtRetirada"));
			...
			if (resp) {
				ret = new Retirada(0, estoque.getIdEstoque(), qtd, produto, obs, new java.sql.Date(dataRet.getTime()));
			...
			e.printStackTrace();
			response.status(203);
			return "<script>alert('Não foi possível dar entrada no produto selecionado - erro: " + e.getMessage()
					+ "'); window.location.href = '" + app.Aplicacao.url + "/pages/home-adicionar.html?id=" + idEstoque
					+ "'</script>";
		}
	}
	...
	public Object entrada(Request request, Response response) {
		...
		try {
			...
			int produto = Integer.parseInt(request.queryParams("produtos"));
			int qtd = Integer.parseInt(request.queryParams("quantidade"));
			java.util.Date dataEnt = dateFormated.parse(request.queryParams("dtentrada"));
			Entrada ent = null;
			Estoque estoque = daoe.get(idEstoque);
			Object prop = new DAOToken().convertTokenCliente(token);
			Fornecedor forn = null;
			...
			if (resp) {
				ent = new Entrada(0, qtd, estoque.getIdEstoque(), 0, new java.sql.Date(dataEnt.getTime()));
